...
def read_csrc_table(payload: bytes) -> dict[str, Any]:
    ...
    for row in best_rows[header_idx + 1:]:
        ...
        records.append({"company": company, "receivedOn": received or None, "status": status or None})
...
def build_report(as_of: date) -> dict[str, Any]:
    ...
    report = {
        ...
        "csrc": {
            ...
            "headers": csrc["headers"],
            "records": csrc["records"],
        },
        "sources": [
            source(HKEX_NLR.format(year=as_of.year), "HKEX New Listing Report", "上市日期、发行价、募资额、保荐人", as_of.isoformat()),
            source(market_url, f"HKEX 每日报价 {market_date}", "最新收市价及年内新股表现", market_date.isoformat()),
            *first_day_sources,
            *pipeline_sources,
            source(csrc_page, "中国证监会境外发行上市备案情况表页面", "备案材料接收与审核进度", as_of.isoformat()),
            source(csrc_xlsx, "中国证监会境外发行上市备案情况表 XLSX", "备案明细原始表", as_of.isoformat()),
        ],
    }
